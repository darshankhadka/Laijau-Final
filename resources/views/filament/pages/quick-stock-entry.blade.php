<x-filament-panels::page>
    <div class="lj-qs-root" x-data="{
        init() {
            ...
            let qsBuffer = '';
            let qsLastKeyTime = Date.now();

            document.addEventListener('keydown', (e) => {
                const activeTag = document.activeElement ? document.activeElement.tagName.toLowerCase() : '';
                const isInput = activeTag === 'input' || activeTag === 'textarea' || activeTag === 'select';
                const isScanInput = document.activeElement === this.$refs.scannerInput;

                const curTime = Date.now();
                const diff = curTime - qsLastKeyTime;
                qsLastKeyTime = curTime;

                if (e.key === 'Enter') {
                    if (isScanInput) {
                        const code = this.$refs.scannerInput ? this.$refs.scannerInput.value.trim() : '';
                        if (code.length >= 2) {
                            e.preventDefault();
                            e.stopPropagation();
                            if (this.$refs.scannerInput) this.$refs.scannerInput.value = '';
                            qsBuffer = '';
                            @this.handleScan(code);
                            return;
                        }
                    } else if (qsBuffer.length >= 2) {
                        e.preventDefault();
                        e.stopPropagation();
                        const code = qsBuffer.trim();
                        qsBuffer = '';
                        if (code.length > 0) {
                            @this.handleScan(code);
                        }
                ...
                <div style="padding: 1rem; display: flex; flex-direction: column; gap: 0.75rem;">
                    ...
                    <div x-show="isInsecureContext" style="background: #fffbeb; border: 1px solid #f59e0b; border-radius: 0.5rem; padding: 0.75rem; color: #92400e; font-size: 0.75rem; line-height: 1.4;">
                        ...
                        <div>
                            ...
                            To use phone cameras, serve via HTTPS or an encrypted tunnel, or enter the SKU / barcode manually.
                        </div>
                    </div>
                    ...
                    <div style="position: relative; width: 100%; min-height: 260px; background: #0f172a; border-radius: 0.5rem; overflow: hidden; display: flex; align-items: center; justify-content: center;">
                        ...
                        <div x-show="hasError" style="position: absolute; inset: 0; background: rgba(15, 23, 42, 0.96); color: #ffffff; padding: 1.5rem; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; gap: 0.75rem; z-index: 10;">
                            ...
                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; justify-content: center;">
                                <button type="button" @click="startCamera()" style="background: #059669; color: #ffffff; padding: 0.45rem 0.95rem; font-size: 0.75rem; font-weight: 700; border-radius: 0.375rem; border: none; cursor: pointer;">
                                    Retry Camera
                                </button>
                                <button type="button" @click="switchCamera()" style="background: rgba(255,255,255,0.15); color: #ffffff; border: 1px solid rgba(255,255,255,0.3); border-radius: 0.375rem; padding: 0.45rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer;">
                                    Try Other Camera
                                </button>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                            <template x-if="cameras.length > 1">
                                <select
                                    ...
                                    style="background: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 0.375rem; padding: 0.35rem 0.6rem; font-size: 0.75rem; font-weight: 600; cursor: pointer; max-width: 170px;">
                    ...
                    async startCamera() {
                        ...
                        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                            ...
                            this.errorMessage = this.isInsecureContext ?
                                'Camera access blocked by browser: Camera requires HTTPS when accessed from a phone/remote device. Please open via HTTPS or enter SKU manually.' :
                        ...
                        try {
                            ...
                            const videoEl = document.querySelector("#lj-stock-camera-viewport video");
                            if (videoEl) {
